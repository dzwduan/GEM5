...
#define RF_GETPRI(x) ((x) & 0b11)
#define RF_GETPID(x) (((x) >> 2) & 0b1111)
#define RF_GETTID(x) ((x) >> 6)

#define RFMAKE_TPI(t, p) (((t) << 4) | (p))
      ...
      inflightIssues(scheduleToExecDelay, 0)
{
    ...
    intRfTPI.resize(outports);
    fpRfTPI.resize(outports);

    bool same_fu = true;
    for (int i = 0; i < outports; i++) {
        for (auto rfp : params.oports[i]->rp) {
            int rf_type = RF_GETTID(rfp);
            int rf_portid = RF_GETPID(rfp);
            int rf_portPri = RF_GETPRI(rfp);
            assert(rf_portPri < 4);
            assert(RFMAKE_TPI(rf_type, rf_portid) < 64);
            if (rf_type == RF_INTID) {
                intRfTPI[i].push_back(std::make_pair(RFMAKE_TPI(rf_type, rf_portid), rf_portPri));
            } else if (rf_type == RF_FPID) {
                fpRfTPI[i].push_back(std::make_pair(RFMAKE_TPI(rf_type, rf_portid), rf_portPri));
            } else {
    ...
    for (int pi = 0; pi < outports; pi++) {
        ...
        if (!readyQ->empty()) {
            ...
            for (int i = 0; i < inst->numSrcRegs(); i++) {
                ...
                std::pair<int, int> rftpi;
                // read port is point to point with srcid
                if (src.isIntReg() && intRfTPI[pi].size() > i) {
                    rftpi = intRfTPI[pi][i];
                    scheduler->useRegfilePort(inst, psrc, rftpi.first, rftpi.second);
                } else if (src.isFloatReg() && fpRfTPI[pi].size() > i) {
                    rftpi = fpRfTPI[pi][i];
                    scheduler->useRegfilePort(inst, psrc, rftpi.first, rftpi.second);
                }
            }
...
Scheduler::Scheduler(const SchedulerParams& params) : SimObject(params), issueQues(params.IQs)
{
    ...
    for (int i = 0; i < issueQues.size(); i++) {
        ...
        for (auto rftpi : issueQues[i]->intRfTPI) {
            for (auto tpi : rftpi) {
                maxTPI = std::max(maxTPI, tpi.first);
                rfportChecker[tpi.first] += 1;
            }
        }
        for (auto rftpi : issueQues[i]->fpRfTPI) {
            for (auto tpi : rftpi) {
                maxTPI = std::max(maxTPI, tpi.first);
                rfportChecker[tpi.first] += 1;
            }
    ...
    assert(maxTPI <= 64);
    rfMaxTPI = maxTPI;
    rfPortOccupancy.resize(maxTPI, {nullptr, 0});
...
bool
Scheduler::checkRfPortBusy(int tpi, int pri)
{
    if (rfPortOccupancy[tpi].first && rfPortOccupancy[tpi].second > pri) {
        return false;
    }
    return true;
}
...
void
Scheduler::useRegfilePort(const DynInstPtr& inst, const PhysRegIdPtr& regid, int tpi, int pri)
{
    ...
    assert(tpi < rfPortOccupancy.size());
    if (rfPortOccupancy[tpi].first) {
        if (rfPortOccupancy[tpi].second > pri) {
            // inst arbitration failure
            arbFailedInsts.push_back(inst);
            DPRINTF(Schedule, "[sn:%llu] arbitration failure, tpi %d occupied by [sn:%llu]\n", inst->seqNum, tpi,
                    rfPortOccupancy[tpi].first->seqNum);
            return;
        } else {
            // rfPortOccupancy[tpi].first arbitration failure
            arbFailedInsts.push_back(rfPortOccupancy[tpi].first);
            DPRINTF(Schedule, "[sn:%llu] arbitration failure, tpi %d occupied by [sn:%llu]\n",
                    rfPortOccupancy[tpi].first->seqNum, tpi, inst->seqNum);
        }
    }
    rfPortOccupancy[tpi] = std::make_pair(inst, pri);
}
